...
-- =======================
-- Ball Modifications Sekmesi 
-- =======================
local BallTab = Window:CreateTab("Ball Modifications", "expand") 
local MainSection = BallTab:CreateSection("Main") 
...
local tpsBalls = {}
local psBalls = {}
local tpsMultiplier = 1
local psMultiplier = 1

-- ⚽ TPS TOPU BÜYÜTME
local function applyToTPSBall(obj)
    if obj:IsA("Part") and obj.Name == "TPS" then
        if not tpsBalls[obj] then
            tpsBalls[obj] = { originalSize = obj.Size }
        ...
        local data = tpsBalls[obj]
        if data then
            obj.Size = data.originalSize * tpsMultiplier
        end
    end
end

-- ⚽ PSoccerBall TOPU BÜYÜTME
local function applyToPSoccerBall(obj)
    if obj:IsA("Part") and obj.Name == "PSoccerBall" then
        if not psBalls[obj] then
            psBalls[obj] = { originalSize = obj.Size }
            obj.AncestryChanged:Connect(function()
                if not obj.Parent then
                    psBalls[obj] = nil
                end
        ...
        local data = psBalls[obj]
        if data then
            obj.Size = data.originalSize * psMultiplier
        end
    end
end

-- Mevcut toplara uygula
for _, obj in pairs(Workspace:GetDescendants()) do
    applyToTPSBall(obj)
    applyToPSoccerBall(obj)
end
...
BallTab:CreateSlider({ 
    ...
    Callback = function(val)
        tpsMultiplier = val
        for obj,_ in pairs(tpsBalls) do
            applyToTPSBall(obj)
        end
    end
})

-- GUI Slider: PSoccerBall
BallTab:CreateSlider({ 
    Name = "PSoccerBall", 
    Range = {1, 10},
    Increment = 1,
